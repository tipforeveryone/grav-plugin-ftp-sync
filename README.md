...
- **Cleanup Hosting**: only scans HOSTING files with mtime after a chosen point in time, then keeps the ones that no longer exist on Local — useful for removing leftover files after renaming/deleting content locally, without re-scanning the entire hosting tree. Unlike Check differences/Push from Local, this does **not** show the full comparison table with per-row Local/Hosting/delete choices — there's nothing to choose here, every result found is always meant to be deleted — it just lists the paths found (folder-collapsed rows shown with a `/**` suffix) under a **"Clean up"** button that deletes exactly that list. Optional **"Collapse by folder"** checkbox: when a whole folder no longer exists on Local (not just one file inside it) AND that folder has no nested subfolder with its own content (e.g. a child page) — a folder that does is always left as individual file rows, never bulk-deleted — collapses every orphaned file under it into a single row that deletes the entire folder on Hosting recursively in one action, instead of listing each file separately. Off by default — a stronger, less granular action, so review the list before "Clean up".
- **Sync now / Clean up**: applies the action(s) above (per-row choice for Check differences/Push from Local, or the flat list for Cleanup Hosting). Whenever Pages was part of that run, clicking either also runs 2 hidden cleanup passes on the whole Pages tree on Hosting — no option to configure, always on, and **not filtered by the "since" mtime cutoff or by whether any file actually got deleted this time** (so it still catches old leftover folders even when nothing else changed and "Cleanup Hosting"'s own scan skipped them for being too old — the button stays clickable even with zero rows/items to review, purely to reach this pass):
...
3. Pick an action in the right panel:
   ...
   - **Cleanup Hosting** → pick a point in time → review the plain list of hosting-only files/folders found → **Clean up** (deletes exactly what's listed, no per-row choice needed).
