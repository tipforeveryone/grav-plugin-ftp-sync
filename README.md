...
- **Push from Local**: only scans LOCAL files with mtime after a chosen point in time, then compares just those against Hosting — faster than "Check differences" when only a few files changed in a large tree. Every row defaults to "Use Local version".
- **Cleanup Hosting**: always applies to **Pages only** (the content-group checkboxes above have no effect on this action — see "why Pages only" below). Only scans HOSTING Pages files with mtime after a chosen point in time, then keeps the ones that no longer exist on Local — useful for removing leftover files after renaming/deleting pages locally, without re-scanning the entire hosting tree. Unlike Check differences/Push from Local, this does **not** show the full comparison table with per-row Local/Hosting/delete choices — there's nothing to choose here, every result found is always meant to be deleted — it just lists the paths found under a **"Clean up pages on hosting"** button that deletes exactly that list.
  - *Why Pages only:* deleting individual leftover files here, combined with the always-on Pages cleanup pass below, already fully removes an orphaned page folder (files + the now-empty directory) regardless of how old its other files are — so there's no need for a separate "delete the whole folder in one action" option for Pages. Themes/Plugins/Config/Accounts don't have that automatic safety net, so bulk-deleting an orphaned folder there was a real (if narrow) foot-gun; scoping this action to Pages removes it entirely instead of relying on users to double-check a destructive list.
- **Sync now / Clean up pages on hosting**: applies the action(s) above (per-row choice for Check differences/Push from Local, or the flat list for Cleanup Hosting). Whenever Pages was part of that run, clicking either also runs 2 hidden cleanup passes on the whole Pages tree on Hosting — no option to configure, always on, and **not filtered by the "since" mtime cutoff or by whether any file actually got deleted this time** (so it still catches old leftover folders even when nothing else changed and "Cleanup Hosting"'s own scan skipped them for being too old — the button stays clickable even with zero rows/items to review, purely to reach this pass):
  1. Deletes, as a whole, any Pages folder on Hosting that no longer contains a `.md` file anywhere inside it (even if it still has other leftover files like images) — a Grav page always needs at least one `.md`, so such a folder can no longer be a valid page or contain one. A folder is only ever removed this way if none of its own subfolders (at any depth) still hold a real page — deleting stops at the first real page found going down, so genuine content and its own asset subfolders are never touched.
  2. Prunes any Pages folder left completely empty as a result (recursively, bottom-up — never the `user/pages` root itself).
...
3. Pick an action in the right panel:
   ...
   - **Push from Local** → pick a point in time → review the list of local files changed since then → **Sync now** (every row already defaults to "Use Local version").
   - **Cleanup Hosting** (Pages only, regardless of the Category selection) → pick a point in time → review the plain list of hosting-only Pages files found → **Clean up pages on hosting** (deletes exactly what's listed, no per-row choice needed).
